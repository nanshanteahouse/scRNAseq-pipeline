...
> 与 `run_pipeline.py` 中的 `STEPS` 保持一致。**注意**: step 02 使用的脚本是 `01_qc.py`（因为 qc 和 doublet 共享同一个输入来源）。
...
> 注意：step 索引为 2，但实际执行的脚本是 `01_qc.py`（与 doublet 步骤共用同一个脚本，内部先去除 doublet 再做 QC 过滤）。
