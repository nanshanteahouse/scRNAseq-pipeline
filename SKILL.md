...
│  01_downsample (可选) → 02_doublet → 03_qc → 04_integrate │
└────────────────────────────────────────────────────┘
    │
    ▼
── INTERACTIVE STOP 1 ──────────────────────────────
...
┌────────────────────────────────────────────────────┐
│  Phase 2: 降维 + 聚类（多参数网格）                   │
│  05_cluster_umap (PCA + UMAP + Leiden grid)         │
└────────────────────────────────────────────────────┘
    │
    ▼
── INTERACTIVE STOP 2 ──────────────────────────────
...
┌────────────────────────────────────────────────────┐
│  Phase 5: 下游分析 + AI 解读                          │
│  08_markers_de → 09_trajectory → 10_enrichment       │
└────────────────────────────────────────────────────┘
    │
    ▼
── INTERACTIVE STOP 5 ──────────────────────────────
...
> **可选步骤**: Step 11 (`06_exploratory.py`) — 探索性分析（组成/QC/marker探索），可在任意阶段之后单独运行。
...
STEPS = [
    ...
    ("01", "downsample.py",             "降采样 (可选, 按配置跳过) → 覆写 00_raw.h5ad"),
    ...
    ("05", "04_cluster_umap.py",        "多参数 UMAP + 多分辨率 Leiden → 04_clustered.h5ad"),
    ("06", "05_annotate_major.py",      "AI 辅助 major cell type 注释 → 05_annotated.h5ad"),
    ("07", "06_subcluster.py",          "交互式亚型分析 (需 --cell-type 参数)"),
    ("08", "07_markers_de.py",          "差异表达分析 (多层级) → marker_genes_per_group.csv"),
    ("09", "08_trajectory.py",          "PAGA + DPT 轨迹分析 → 05_final.h5ad"),
    ("10", "09_enrichment.py",          "GO/KEGG 富集 + AI 解读 → enrichment_*.csv"),
    ("11", "06_exploratory.py",         "探索性分析 (组成/QC/marker探索，可选)"),
]
```
...
是否需要降采样以控制内存使用？
  [跳过] — 数据集不大，直接继续
  [降采样到 50000] — 保留 50000 个细胞（按样本分层采样）
  [自定义] — 指定目标细胞数或采样策略
```

- 用户选择跳过 → 确保 config 中 `downsample_target = None`（默认值），Step 01 自动跳过
- 用户选择降采样 → 在 config 中设置 `downsample_target` 等参数

```bash
python run_pipeline.py --step 1 --config config_project.py
```

- 输出：覆写 `00_raw.h5ad`（降采样后的数据）
- 支持策略：`random`, `stratified`（默认，按样本分层）, `max_per_sample`
- 运行 `--target-fraction` 或 `--target-total` 或 `--max-per-sample` 独立使用

### Step 02 — doublet（Per sample Scrublet）
...
1. 自动读取 step 02 (doublet) 和 03 (QC) 的输出日志/结果文件
2. 向用户展示以下信息：
...
- 用户调整阈值 → 修改 config → rerun step 3 → 回到此 stop
...
```bash
python run_pipeline.py --step 6 --config config_project.py
```
